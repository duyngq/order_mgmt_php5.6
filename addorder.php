...
//	$orderDate = $_POST ["orderDate"];
	...
	$checkCustomerResult = mysqli_query ($connection, $checkCustomerQuery ) or die ( mysqli_error ($connection) . "Can not retrieve database" );

	$checkReceiverQuery = "select id, cust_name from recvcustomers where phone='" . $recvPhone . "'";
    $checkReceiverResult = mysqli_query ($connection, $checkReceiverQuery ) or die ( mysqli_error ($connection) . "Can not retrieve database" );
	...
	if ($checkCustomerResult && $checkReceiverResult) {
		if ( $checkCustomerResult->num_rows == 0) {
			...
			$addCustomerResult = mysqli_query ( $connection, $addCustomerQuery ) or die ( mysqli_error ($connection) . "Can not retrieve to database" );
		...
		if ( $checkReceiverResult->num_rows == 0) {
            ...
            $addReceiverResult = mysqli_query ( $connection, $addReceiverQuery ) or die ( mysqli_error ($connection) . "Can not retrieve to database" );
            $recvCustId = mysqli_insert_id ($connection);
        ...
        if (is_null($custId)) {
        	while ( $customer = mysqli_fetch_array ( $checkCustomerResult ) ) {
        		$custId = $customer ["id"];
        	}
        }

        if (is_null($recvCustId)) {
        	while ( $customer = mysqli_fetch_array ( $checkReceiverResult ) ) {
...
//	$orderDate = $dates[2]."-".$dates[1]."-".$dates[0];
	...
	$addNewOrderResult = mysqli_query ( $connection, $addNewOrder ) or die ( mysqli_error ($connection) . "Can not retrieve to database" );
...
function addOrderDetails($orderId, $products, $connection, $submit) {
	...
	$addOrderDetailsResult = mysqli_query ( $connection, substr($addOrderDetails, 0, -1) ) or die ( mysqli_error () . "Can not retrieve to database" );
...
function getSenders() {
    ...
    $getCustomersNameResult = mysqli_query($connection, $getCustomersNameQuery);
    // Mysql_num_row is counting table row
    if($getCustomersNameResult) {
        while ($customer=mysqli_fetch_array($getCustomersNameResult)) {
...
function getReceivers() {
    ...
    $getRecvNameResult = mysqli_query($connection, $getRecvNameQuery);
    // Mysql_num_row is counting table row
    if($getRecvNameResult) {
        while ($recv = mysqli_fetch_array($getRecvNameResult)) {
