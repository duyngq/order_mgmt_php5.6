...
//    $orderDate = $_POST ["orderDate"];
	...
	if ($setClauseForUpdateCustomerQuery != null || $setClauseForUpdateCustomerQuery != '') {
		...
		$updateCustomerResult = mysqli_query($connection, $updateCustomerQuery) or die(mysqli_error() . "Can not store data to database");
	...
	if ($setClauseForUpdateRecvQuery != null || $setClauseForUpdateRecvQuery != '') {
		...
		$updateRecvResult = mysqli_query($connection, $updateRecvQuery) or die(mysqli_error() . "Can not store data to database");
		if (!$updateRecvResult) {
			rollback();
			clearAll ( $connection, $sbmUpdateInfo );
    ...
    if ($setClauseForUpdateOrderQuery != null || $setClauseForUpdateOrderQuery != '') {
        ...
        $updateOrderResult = mysqli_query($connection, $updateOrderQuery) or die(mysqli_error() . "Can not store data to database");
    ...
    if (isset($_SESSION['oldOrderDetailsArray'])) { // update existing data
        ...
        foreach ($products as $product) {
	        ...
	        $updateOrderDetailsResult = mysqli_query($connection, $updateOrderDetailsQuery) or die(mysqli_error() . "Can not store data to database");
    ...
    if ($addSysLogCommentQuery != "INSERT INTO comments(date, comment, order_id, user_name) VALUES") {
    	$addSysLogCommentResult = mysqli_query($connection, $addSysLogCommentQuery) or die(mysqli_error() . "Can not store comment to database");
	...
	if ($addCommentQuery != "INSERT INTO comments(date, comment, order_id, user_name) VALUES") {
		$addCommentResult = mysqli_query($connection, $addCommentQuery) or die(mysqli_error() . "Can not store comment to database");
		if (!$addCommentResult) {
			rollback();
            clearAll ( $connection, $sbmUpdateInfo );
			exit;
...
<table width="1024px" border="1">
	<tr>
		...
		if (mysqli_num_rows($ordersResult) < 1) {
		...
		$custResult = mysqli_query($connection, $getCustQuery) or die(mysqli_error() . "Can not retrieve information from database");
		while ($cust = mysqli_fetch_array($custResult)) {
			...
			$recvResult = mysqli_query($connection, $getRecvQuery) or die(mysqli_error() . "Can not retrieve information from database");
			while ($recv = mysqli_fetch_array($recvResult)) {
                        ...
                        <td width=60% style="vertical-align: top;" id="feeTableRow">
                            ...
                            <?php
                                if ($_SESSION['newType'] == 1) { // new order type --> retrieve to orderdetails
                                    ...
                                    $orderDetailsResult = mysqli_query($connection, $getOrderDetailsQuery) or die(mysqli_error() . "Can not retrieve information from database");
                                    $noOfProducts = 0;
                                    $oldProducts = array ();
                                    while ($orderDetails = mysqli_fetch_array($orderDetailsResult)) {
                                        $product = array ();
